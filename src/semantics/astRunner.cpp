...
namespace Snapp {
    ...
    std::optional<DataValue> ASTRunner::runASTNode(const SyntaxNode *node) {
        ...
        if (auto* functionCall = dynamic_cast<const SyntaxNodeFunctionCall*>(node)) {
            ...
            if (auto* nativeFunction = std::get_if<NativeFunctionValue>(&callee)) {
            ...
            } else if (auto* function = std::get_if<FunctionValue>(&callee)) {
                ...
                auto returnValue = runASTNode(function->body);
                scopeIndex_ = parent;
                return returnValue;
            } else {
